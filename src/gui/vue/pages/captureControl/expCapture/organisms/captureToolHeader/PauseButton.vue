...
<template>
  <div>
    ...
    <v-layout row justify-centre>
      <v-dialog
        v-model="ConfirmationModelVisisbility"
        presistent
        max-width="290"
      >
        <v-card>
          <v-card-title class="error headline" style="font-weight: bold">{{
            $store.getters.message("confirmation_dialog_box.card-title")
          }}</v-card-title>
          <v-card-text>{{
            $store.getters.message("confirmation_dialog_box.card-content")
          }}</v-card-text>
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn flat @click="ConfirmationModelVisisbility = false">{{
              $store.getters.message("confirmation_dialog_box.card-button-1")
            }}</v-btn>
            <v-btn
              color="error"
              flat
              @click="pushPauseButton"
              v-on:click="ConfirmationModelVisisbility = false"
              >{{
                $store.getters.message("confirmation_dialog_box.card-button-2")
              }}</v-btn
            >
          </v-card-actions>
        </v-card>
